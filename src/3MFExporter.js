...
async function import_decompress() {
  ...
  try {
    const { CanvasTexture, NodeMaterial, QuadMesh, WebGPURenderer } =
      await import('three');
...
class ThreeMFExporter {
    ...
    scene.traverse((object) => {
      if (object.isMesh) {
        ...
        if (material.map) {
          ...
          let name = material.map.name
            ? material.map.name
            : 'texture_' + material.map.uuid;
          ...
          if (geometry.hasAttribute('uv')) {
            resourcesString += this.generateUVs(
              geometry,
              object.id,
              material.map.id
            );
        ...
        resourcesString += this.generateTriangles(
          geometry,
          material.map ? object.id : null
        );
        resourcesString += '   </mesh>\n';
  ...
  async createTexturesRelsFile(scene) {
    ...
    scene.traverse((object) => {
      if (object.isMesh && object.material.map) {
        map_found = true;
        let name = object.material.map.name
          ? object.material.map.name
          : 'texture_' + object.material.map.uuid;
        if (name.indexOf('.') === -1) name += '.png';
        ...
        if (!image_names[name]) {
          ...
          relsStringTextures +=
            ' <Relationship Target="/3D/Textures/' +
            name +
            '" Id="rel' +
            object.material.map.id +
  ...
  generateUVs(geometry, id, texid) {
    ...
    let uvsString =
      '  <m:texture2dgroup id="' + id + '" texid="' + texid + '">\n';
  ...
  imageToCanvas(image, flipY, maxTextureSize) {
    if (
      ...
      throw new Error(
        'THREE.3MFExporter: No valid image data found. Unable to process texture.'
      );
    }
  }
}
